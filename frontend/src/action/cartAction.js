...
export function addToCartAction(quantityBuy, productId, setIsAddedToCart) {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axiosInstance.post("/api/cart/add_to_cart", {
        userId,
        quantityBuy,
        productId,
      });
...
export const reduceQttAction =
  (userId, productId, setIsLoadingChangeQtt) => async (dispatch) => {
    try {
      const response = await axiosInstance.put("/api/cart/reduce_qtt", {
        userId,
        productId,
      });
...
export const increaseQttAction =
  (userId, productId, setIsLoadingChangeQtt) => async (dispatch) => {
    try {
      const response = await axiosInstance.put("/api/cart/increase_qtt", {
        userId,
        productId,
      });
...
export const deleteProductAction = (userId, productId) => async (dispatch) => {
  try {
    const response = await axiosInstance.delete("/api/cart/delete_product", {
...
export const orderAction = (userId, setIsOrder) => async (dispatch) => {
  try {
    const response = await axiosInstance.put("/api/cart/add_to_order", {
      userId,
    });
