...
export const getFlashSaleAction = () => async (dispatch) => {
  try {
    const response = await axiosInstance.get("/api/products/flash_sale", {
      headers: {
        'is-next': true,
      },
    });
...
export const getAllProductsSellerAction = (userId) => async (dispatch) => {
  try {
    const response = await axiosInstance.get("/api/products/seller/get_all", {
      params: {
        userId,
      },
    });
  ...
  (form, setError, setIsAddSuccess) => async (dispatch) => {
    try {
      const response = await axiosInstance.post(
        "/api/products/seller/add_product",
        form,
        {
          headers: {
...
export async function getAllProduct(setAllProducts) {
  try {
    const response = await axios.get("/api/products/get_all_products_customer", {
      headers: {
        'is-next': true
      }
    });
